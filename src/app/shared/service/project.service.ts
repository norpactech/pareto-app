...
import { IProjectPostDTO, IProjectPutDTO, IProjectDeleteDTO } from '@shared/dto'
...
export class ProjectService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/project';

  public get(id: string): Observable<IProject | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IProject>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IProject[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IProject[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IProjectPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IProjectPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IProjectDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
