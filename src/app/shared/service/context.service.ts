...
import { IContextPostDTO, IContextPutDTO, IContextDeleteDTO } from '@shared/dto'
...
export class ContextService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/context'

  public get(id: string): Observable<IContext | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IContext>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IContext[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IContext[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IContextPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IContextPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IContextDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
