...
import { IProjectComponentProperty } from '@shared/model'
import { IProjectComponentPropertyPostDTO, IProjectComponentPropertyPutDTO, IProjectComponentPropertyDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class ProjectComponentPropertyService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/project-component-property'

  public get(id: string): Observable<IProjectComponentProperty | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IProjectComponentProperty>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IProjectComponentProperty[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IProjectComponentProperty[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IProjectComponentPropertyPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IProjectComponentPropertyPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IProjectComponentPropertyDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
