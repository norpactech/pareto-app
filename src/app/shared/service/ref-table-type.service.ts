...
import { IRefTableTypePostDTO, IRefTableTypePutDTO, IRefTableTypeDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class RefTableTypeService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/ref-table-type'

  public get(id: string): Observable<IRefTableType | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IRefTableType>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IRefTableType[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IRefTableType[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IRefTableTypePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IRefTableTypePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IRefTableTypeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
