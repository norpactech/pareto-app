...
export class GenericDataTypeService extends BaseService {
  ...
  public get(id: string): Observable<IGenericDataType | null> {
    ...
    return this.httpClient.get<IApiResponse<IGenericDataType>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IGenericDataType[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IGenericDataType[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IGenericDataTypePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IGenericDataTypePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IGenericDataTypeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
