...
import { IGenericDataTypeAttributePostDTO, IGenericDataTypeAttributePutDTO, IGenericDataTypeAttributeDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class GenericDataTypeAttributeService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/generic-data-type-attribute'

  public get(id: string): Observable<IGenericDataTypeAttribute | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IGenericDataTypeAttribute>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IGenericDataTypeAttribute[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IGenericDataTypeAttribute[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IGenericDataTypeAttributePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IGenericDataTypeAttributePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IGenericDataTypeAttributeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
