...
import { IRefTables } from '@shared/model'
import { IRefTablesPostDTO, IRefTablesPutDTO, IRefTablesDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class RefTablesService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/ref-tables'

  public get(id: string): Observable<IRefTables | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IRefTables>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IRefTables[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IRefTables[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IRefTablesPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IRefTablesPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IRefTablesDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
