...
import { ISchemaPostDTO, ISchemaPutDTO, ISchemaDeleteDTO } from '@shared/dto'
...
export class SchemaService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/schema'

  public get(id: string): Observable<ISchema | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<ISchema>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: ISchema[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<ISchema[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: ISchemaPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: ISchemaPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: ISchemaDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
