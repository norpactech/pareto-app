...
import { IGenericPropertyType } from '@shared/model'
import { IGenericPropertyTypePostDTO, IGenericPropertyTypePutDTO, IGenericPropertyTypeDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class GenericPropertyTypeService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/generic-property-type'

  public get(id: string): Observable<IGenericPropertyType | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IGenericPropertyType>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IGenericPropertyType[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IGenericPropertyType[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IGenericPropertyTypePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IGenericPropertyTypePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IGenericPropertyTypeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
