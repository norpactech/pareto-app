...
import { IContextPropertyType } from '@shared/model'
import { IContextPropertyTypePostDTO, IContextPropertyTypePutDTO, IContextPropertyTypeDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class ContextPropertyTypeService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/context-property-type'

  public get(id: string): Observable<IContextPropertyType | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IContextPropertyType>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IContextPropertyType[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IContextPropertyType[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IContextPropertyTypePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IContextPropertyTypePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IContextPropertyTypeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
