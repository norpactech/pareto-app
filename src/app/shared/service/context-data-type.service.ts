...
import { IContextDataType } from '@shared/model'
import { IContextDataTypePostDTO, IContextDataTypePutDTO, IContextDataTypeDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class ContextDataTypeService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/context-data-type'

  public get(id: string): Observable<IContextDataType | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IContextDataType>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IContextDataType[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IContextDataType[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IContextDataTypePostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IContextDataTypePutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IContextDataTypeDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
