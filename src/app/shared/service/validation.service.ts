...
import { IValidationPostDTO, IValidationPutDTO, IValidationDeleteDTO } from '@shared/dto'
...
export class ValidationService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/validation'

  public get(id: string): Observable<IValidation | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IValidation>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IValidation[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IValidation[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IValidationPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IValidationPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IValidationDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
