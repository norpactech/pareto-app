...
import { ICardinality } from '@shared/model'
import { ICardinalityPostDTO, ICardinalityPutDTO, ICardinalityDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class CardinalityService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/cardinality'

  public get(id: string): Observable<ICardinality | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<ICardinality>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: ICardinality[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<ICardinality[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: ICardinalityPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: ICardinalityPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: ICardinalityDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
