...
import { IDataIndexProperty } from '@shared/model'
import { IDataIndexPropertyPostDTO, IDataIndexPropertyPutDTO, IDataIndexPropertyDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class DataIndexPropertyService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService)
  protected readonly baseUrl = this.environmentService.apiUrl + '/data-index-property'

  public get(id: string): Observable<IDataIndexProperty | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IDataIndexProperty>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IDataIndexProperty[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IDataIndexProperty[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IDataIndexPropertyPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IDataIndexPropertyPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IDataIndexPropertyDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
