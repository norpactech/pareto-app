...
import { IDataObjectPostDTO, IDataObjectPutDTO, IDataObjectDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class DataObjectService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService);
  protected readonly baseUrl = this.environmentService.apiUrl + '/data-object';

  public get(id: string): Observable<IDataObject | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IDataObject>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IDataObject[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IDataObject[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IDataObjectPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IDataObjectPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IDataObjectDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
