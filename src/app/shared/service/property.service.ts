...
import { IProperty } from '@shared/model'
import { IPropertyPostDTO, IPropertyPutDTO, IPropertyDeleteDTO } from '@shared/dto'
...
export class PropertyService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/property'

  public get(id: string): Observable<IProperty | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IProperty>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IProperty[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IProperty[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IPropertyPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IPropertyPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IPropertyDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
