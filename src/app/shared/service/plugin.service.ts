...
import { IPluginPostDTO, IPluginPutDTO, IPluginDeleteDTO } from '@shared/dto'
...
export class PluginService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/plugin'

  public get(id: string): Observable<IPlugin | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IPlugin>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IPlugin[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IPlugin[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IPluginPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IPluginPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
  ...
  public delete(data: IPluginDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, unknown> = this.getRequestData(data)
