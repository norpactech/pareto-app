...
import { IUserPostDTO, IUserPutDTO, IUserDeleteDTO } from '@shared/dto'
...
export class UserService extends BaseService {
  ...
  protected readonly baseUrl = this.environmentService.apiUrl + '/user'

  public get(id: string): Observable<IUser | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IUser>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IUser[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IUser[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IUserPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IUserPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IUserDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
