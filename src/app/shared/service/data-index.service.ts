...
import { IDataIndex } from '@shared/model'
import { IDataIndexPostDTO, IDataIndexPutDTO, IDataIndexDeleteDTO } from '@shared/dto'

@Injectable({
  providedIn: 'root',
})
export class DataIndexService extends BaseService {

  protected readonly httpClient = inject(HttpClient)
  protected readonly snackBar = inject(MatSnackBar)
  protected readonly environmentService = inject(EnvironmentService);
  protected readonly baseUrl = this.environmentService.apiUrl + '/data-index';

  public get(id: string): Observable<IDataIndex | null> {

    // TODO: Input Validation on the Primary Key - Singular or Compound

    return this.httpClient.get<IApiResponse<IDataIndex>>(`${this.baseUrl}?id=${id}`).pipe(
      map((response) => {
        if (!response.data) {
          if (response.error) {
  ...
  public find(params: Record<string, unknown>): Observable<{ data: IDataIndex[]; total: number }> {
    ...
    return this.httpClient
      .get<IApiResponse<IDataIndex[]>>(`${this.baseUrl}/find`, { params: queryParams })
      .pipe(
        map((response) => ({
          data: response.data ?? [],
  ...
  public create(data: IDataIndexPostDTO): Observable<IPersistResponse> {
    if (!data) {
  ...
  public update(data: IDataIndexPutDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
  ...
  public delete(data: IDataIndexDeleteDTO): Observable<IPersistResponse> {
    if (!data) {
      return throwError(() => new Error('Null or undefined data'))
    }
    const requestBody: Record<string, string> = this.getRequestData(data)
